...
class BlockClass extends React.Component {
    ...
    */
    handleMouseDown = event => {
        ...
        const { left, top } = this.handleRef.getBoundingClientRect(); //position of the box relative to the view - changes
        this.dragStartLeft = left - offsetLeft;
        this.dragStartTop = top - offsetTop;
        this.dragStartX = clientX;
        this.dragStartY = clientY;
        if(!this.props.dragZone){ //Drop area
            this.props.parentCallbackRefBlock(this.handleRef)
        } 
    ...
    */  
    checkBoundingMove(x,y){
        ...
        var bounding = this.handleRef.getBoundingClientRect()
        ...
        if(!this.props.dragZone ){           
            if(left<drop.offsetLeft) //Left border of drop-zone
                this.handleRef.style.transform = `translate(${x+(drop.offsetLeft+10-left)}px, ${y}px)`
            if(right>offsetRight) //right border of drop-zone = offsetleft+offsetright            
                this.handleRef.style.transform = `translate(${x-(right-offsetRight+20)}px, ${y}px)`
            if(top<drop.offsetTop) //sottrarre da y quanto sto sforando
                this.handleRef.style.transform = `translate(${x}px, ${y-(top-drop.offsetTop-10)}px)`
            if(bottom>drop.offsetHeight)//aggiungere il tanto che sto sforando
                this.handleRef.style.transform = `translate(${x}px, ${y+(drop.offsetHeight-bottom-20)}px)`
            if(left<drop.offsetLeft && top<drop.offsetTop) // Left-top
                this.handleRef.style.transform = `translate(${x+(drop.offsetLeft+10-left)}px, ${y-(top-drop.offsetTop-10)}px)`
            if(right>offsetRight && top<drop.offsetTop) //Right-top
                this.handleRef.style.transform = `translate(${x-(right-offsetRight+20)}px, ${y-(top-drop.offsetTop-10)}px)`
            if(bottom>drop.offsetHeight&&left<drop.offsetLeft) //Bottom-left
                this.handleRef.style.transform = `translate(${x+(drop.offsetLeft+10-left)}px, ${y+(drop.offsetHeight-bottom-20)}px)`
            if(bottom>drop.offsetHeight&&right>offsetRight) //Bottom-right
            this.handleRef.style.transform = `translate(${x-(right-offsetRight+20)}px, ${y+(drop.offsetHeight-bottom-20)}px)`            
            this.props.parentCallbackOnMove(parseTransform(this.handleRef.style.transform))
        }    
    }
    ...
    handleMouseMove = ({ clientX, clientY }) => {
        ...
        this.handleRef.style.transform = `translate(${x}px, ${y}px)`;
    ...
    handleMouseUp = (ev) => {
        ...
        if(this.props.dragZone){
            if(ev.clientX>dragZoneWidht){
                var x_y = parseTransform(this.handleRef.style.transform)
                ...
                this.props.parentCallbackAttachToDrop(parseFloat(this.handleRef.id.split('-')[2]), style, this.props.block_type)
            }else this.props.parentCallbackDeleteDragBlock(parseFloat(this.handleRef.id.split('-')[2]))
        }       
